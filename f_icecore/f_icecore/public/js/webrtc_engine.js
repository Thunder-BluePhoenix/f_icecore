...
class FIceCoreWebRTC {
	...
	async startCall(remoteUser, callType = 'audio', callId = null) {
		try {
			...
			await frappe.call({
				method: 'f_icecore.api.signaling.send_offer',
				args: {
					to_user: remoteUser,
	...
	async handleRemoteOffer(data) {
		try {
			...
			// Send answer back
			await frappe.call({
				method: 'f_icecore.api.signaling.send_answer',
				args: {
	...
	async sendIceCandidate(candidate) {
		try {
			await frappe.call({
				method: 'f_icecore.api.signaling.send_ice_candidate',
				args: {
	...
	startPresenceHeartbeat() {
		...
		setInterval(() => {
			frappe.call({
				method: 'f_icecore.api.presence.heartbeat',
				args: {},
				callback: (r) => {
