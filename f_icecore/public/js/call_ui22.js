...
class FIceCoreCallUI {
	...
	setupIncomingCallListener() {
		...
		// Listen for incoming calls (like f_chat does - simple event name, broadcast to all)
		...
		frappe.realtime.on('call_accepted', (data) => {
			...
			if (data.accepted_by === user || data.call_id) {
		...
		frappe.realtime.on('call_rejected', (data) => {
			...
			// Only process if this involves us
			if (data.rejected_by === user || data.call_id) {
				this.stopCallingTone();
				frappe.show_alert({
