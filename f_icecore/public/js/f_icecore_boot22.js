...
f_icecore.init_navbar = function() {
	...
	$btn.find('a').on('click', function(e) {
		...
		setTimeout(() => {
			...
			// Load content with pending calls section
			let contentHtml = '';

			// Show pending incoming calls first
			if (pendingCalls.length > 0) {
				contentHtml += `
					<div style="padding: 20px; background-color: #fff3cd; border-bottom: 2px solid #ffc107;">
			...
			if (pendingCalls.length > 0) {
				const $pendingList = d.$wrapper.find('#f-ic-pending-calls');
				let pendingHtml = '';

				pendingCalls.forEach(call => {
					const callTypeIcon = call.call_type === 'video' ? '📹' : '📞';
					pendingHtml += `
						<div style="padding: 15px; border: 2px solid #ffc107; border-radius: 8px; margin-bottom: 10px; background-color: white;">
							<div style="display: flex; align-items: center; justify-content: space-between;">
								<div style="flex: 1;">
									<div id="incoming-avatar-${call.call_id}" style="display: inline-block; margin-right: 10px;"></div>
									<div style="display: inline-block; vertical-align: top;">
										<strong style="font-size: 16px;">${call.from_user_name}</strong>
										<br>
										<span style="color: #888; font-size: 14px;">${callTypeIcon} ${call.call_type.charAt(0).toUpperCase() + call.call_type.slice(1)} Call</span>
									</div>
								</div>
								<div style="white-space: nowrap;">
									<button class="btn btn-success btn-lg"
										onclick="window.FIceCoreUI.acceptCallFromNotification('${call.call_id}', '${call.from_user}', '${call.call_type}'); return false;"
										style="margin-right: 10px;">
										<i class="fa fa-phone"></i> Accept
									</button>
									<button class="btn btn-danger btn-lg"
										onclick="window.FIceCoreUI.rejectCallFromNotification('${call.call_id}'); return false;">
				...
				// Render avatars for pending calls
				pendingCalls.forEach(call => {
					const avatarContainer = d.$wrapper.find(`#incoming-avatar-${call.call_id}`)[0];
					if (avatarContainer) {
						avatarContainer.innerHTML = frappe.avatar(call.from_user, 'avatar-small');
					}
				});
			}
