...
class FIceCoreCallUI {
	...
	setupIncomingCallListener() {
		...
		console.log('🎧 F-IceCore: Setting up incoming call listener for user:', user);

		// Listen for incoming calls (user-specific event)
		frappe.realtime.on('f_icecore:incoming_call', (data) => {
			console.log('📞 F-IceCore: Incoming call (user-specific) from:', data.from_user);
		...
		// Also listen to broadcast event as fallback
		frappe.realtime.on('f_icecore:incoming_call_broadcast', (data) => {
			console.log('📞 F-IceCore: Incoming call (broadcast) from:', data.from_user);
			console.log('📞 F-IceCore: Call data:', data);

			// Only show if we're the recipient and NOT the caller
			if (data.to_user === user && data.from_user !== user) {
				this.showIncomingCallDialog(data);
				this.playRingtone();
			}
		});
