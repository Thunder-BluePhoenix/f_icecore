/**
 * F-IceCore Bundle v24
 * Main entry point for F-IceCore client-side code
 *
 * FIXES:
 * - Fixed "Cannot set properties of undefined (setting 'showCallMenu')" error
 * - Ensures window.FIceCoreUI is initialized before extending it
 *
...
function addCallMenu() {
	...
	if (frappe.boot.user && frappe.boot.user.name !== 'Guest') {
		...
		if (navbar.length) {
			...
			const callButton = $(`
				<li>
					<a href="#" onclick="if (window.FIceCoreUI && typeof window.FIceCoreUI.showCallMenu === 'function') { window.FIceCoreUI.showCallMenu(); } else { alert('Call UI not ready yet, please wait...'); } return false;"
	...
	const dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				fieldtype: 'Check',
				fieldname: 'is_urgent',
				label: __('Urgent Call'),
				default: 0,
				description: __('Mark as urgent - recipient will see popup immediately')
			}
		],
		primary_action_label: __('Call'),
		primary_action: (values) => {
			const callType = values.call_type.toLowerCase();
			if (window.FIceCoreUI && typeof window.FIceCoreUI.initiateCall === 'function') {
				window.FIceCoreUI.initiateCall(values.user, callType, values.is_urgent);
			} else {
				frappe.msgprint(__('Call UI not ready yet'));
			}
			dialog.hide();
		}
	});
...
// 🆕 FIXED: Wait for FIceCoreUI to be initialized before extending it
function extendFIceCoreUI() {
	if (!window.FIceCoreUI) {
		console.warn('F-IceCore Bundle: FIceCoreUI not ready yet, retrying...');
		setTimeout(extendFIceCoreUI, 100);
		return;
	}

	console.log('F-IceCore Bundle: Extending FIceCoreUI with showCallMenu...');

	// Override the placeholder showCallMenu method
	...
	console.log('✅ F-IceCore Bundle: showCallMenu extended successfully');
}

// Start the extension process
setTimeout(extendFIceCoreUI, 200);
		...
		const userCard = $(`
			<div class="f-icecore-user-card">
				...
				<div class="call-actions">
					<button class="audio" onclick="window.FIceCoreUI.initiateCall('${user.user}', 'audio', false)" title="${__('Audio Call')}">
						<i class="fa fa-phone"></i>
					</button>
					<button class="video" onclick="window.FIceCoreUI.initiateCall('${user.user}', 'video', false)" title="${__('Video Call')}">
						<i class="fa fa-video"></i>
					</button>
					<button class="urgent" onclick="window.FIceCoreUI.initiateCall('${user.user}', 'audio', true)" title="${__('Urgent Call')}" style="background: #ff4444; color: white;">
...
function renderCallHistory(calls) {
	...
	calls.forEach(call => {
		...
		const urgentBadge = call.is_urgent ? '<span style="color: #ff4444; font-weight: bold;">🚨 URGENT</span> ' : '';
		...
		const callItem = $(`
			<div class="f-icecore-call-history-item">
				...
				<div class="call-info">
					<div class="call-user">${urgentBadge}${call.other_user_name}</div>
					<div class="call-meta">
