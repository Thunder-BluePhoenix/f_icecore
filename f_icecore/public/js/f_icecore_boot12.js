					...
					r.message.forEach(user => {
						html += `
							<div style="padding: 10px; border-bottom: 1px solid #ddd;">
								...
								<span style="color: #888; margin-left: 10px;">${user.status}</span>
								<button class="btn btn-primary btn-sm" style="float: right;" onclick="alert('Call ${user.full_name}')">
									<i class="fa fa-phone"></i> Call
								</button>
							</div>
						`;
					});
					$('#f-ic-users-list').html(html);
				} else {
					$('#f-ic-users-list').html(`
